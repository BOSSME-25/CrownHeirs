...
export async function POST() {
  ...
  try {
    ...
    // Seed starter compliance items (flagged for verification) — only if empty.
    {
      ...
      if (Number(n) === 0) {
        const orgId =
          ((await sql`SELECT id FROM organizations WHERE slug = 'crown-heirs' LIMIT 1`) as { id: string }[])[0]?.id ?? null;
        const seed: [string, string, string, string][] = [
          // level, title, cadence, description
          ["federal", "Form I-9 on file for every employee", "once", "Employment eligibility verification completed and retained per federal rules. Verify current requirements with counsel."],
          ["federal", "Form W-4 on file for every employee", "once", "Signed W-4 collected for payroll withholding. Verify with counsel."],
          ["federal", "Federal labor-law posters displayed", "annual", "FLSA, EEO, FMLA, OSHA and other required posters current and posted. Verify the required poster set."],
          ["federal", "Wage & hour (FLSA) records maintained", "annual", "Minimum wage, overtime and timekeeping records retained. Verify with counsel."],
          ["federal", "Anti-harassment / EEO policy distributed & acknowledged", "annual", "Policy provided to staff and acknowledged. Verify with counsel."],
          ["federal", "OSHA workplace safety & injury records", "annual", "Safety data and any injury log maintained. Verify applicability."],
          ["state", "Arizona minimum wage poster displayed", "annual", "Current AZ minimum wage notice posted. Verify current rate/poster."],
          ["state", "Workers' compensation coverage active", "annual", "Active workers' comp policy on file. Verify renewal date."],
          ["state", "Arizona paid sick time accrual tracked", "annual", "Earned paid sick time under the AZ Fair Wages & Healthy Families Act tracked. Verify with counsel."],
          ["state", "New-hire reporting to Arizona", "once", "New hires reported to the state within the required window. Verify current process."],
          ["board", "Salon / establishment license current & posted", "biennial", "AZ Board of Barbering & Cosmetology establishment license active and posted. Verify renewal date."],
          ["board", "Every practitioner's license current & posted", "annual", "Each stylist's license current and displayed (see Licenses & Certifications). Verify."],
          ["board", "Disinfection / sanitation logs maintained", "monthly", "Barbicide / disinfection practices and logs maintained per Board rules. Verify requirements."],
          ["board", "Inspection records retained", "annual", "Board inspection reports kept on file. Verify retention period."],
          ["board", "Client complaint / incident log", "annual", "Log of client complaints and incidents maintained. Verify with Board/counsel."],
        ];
        for (let i = 0; i < seed.length; i++) {
          const [level, title, cadence, description] = seed[i];
          await sql`
            INSERT INTO compliance_items (org_id, level, title, description, cadence, status, needs_verification, sort_order)
            VALUES (${orgId}, ${level}, ${title}, ${description}, ${cadence}, 'attention', true, ${i})
          `;
        }
      }
    }
