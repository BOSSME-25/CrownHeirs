...
async function createAppointment({ serviceSlug, variationId = null, stylistSlug = 'any', startAt, client, notes = '', staff = false, now = new Date() }) {
  ...
  // The booking is committed; messages are best-effort and never fail it.
  appt.notified = await notify.safely(
    notify.booked({ ...appt, client: { name: who.name, phone: who.phone, email: who.email } }, settings.time_zone)
  );
  return appt;
}
...
async function cancel(code, now = new Date()) {
  ...
  const notified = await notify.safely(notify.cancelled({
    code: full.code, startsAt: full.starts_at,
    service: { name: full.service_name }, variation: { name: full.variation_name },
    stylist: { name: full.stylist_name },
    client: { name: full.client_name, phone: full.phone, email: full.email }
  }, settings.time_zone));
  return { code: a.code, status: 'cancelled', notified };
}
