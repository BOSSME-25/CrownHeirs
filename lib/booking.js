...
const { localToUtc, addDays, todayIn } = require('./tz');
...
async function listServices() {
  const { rows } = await query(
    `SELECT slug, name, category, description, price_from_cents, duration_min, requires_consult
...
/**
 * Slots for a service on a date, per stylist plus a merged "any stylist" list.
 */
async function availability({ serviceSlug, date, stylistSlug = null, now = new Date() }) {
  ...
  if (!stylists.length) return { service: publicService(service), date, stylists: [], any: [] };
  ...
  return { service: publicService(service), date, stylists: perStylist, any: [...anySet].sort() };
}
 ...
 */
async function createAppointment({ serviceSlug, stylistSlug = 'any', startAt, client, notes = '', now = new Date() }) {
  const settings = await getSettings();
  const service = await getService(serviceSlug);
  const start = new Date(startAt);
  if (isNaN(start)) throw new BookingError(400, 'Invalid start time');
  const local = require('./tz').utcToLocal(start, settings.time_zone);
  ...
  const endsAt    = new Date(start.getTime() + service.duration_min * 60000);
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        const { rows: [a] } = await c.query(
          `INSERT INTO appointments (code, stylist_id, service_id, client_id, starts_at, ends_at, busy_until, notes)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id, code, starts_at, ends_at`,
          [code, stylist.id, service.id, cl.id, start, endsAt, busyUntil, notes]
        );
        return {
          code: a.code, startsAt: a.starts_at, endsAt: a.ends_at,
          stylist: { slug: stylist.slug, name: stylist.name },
          service: publicService(service), client: { name: who.name }
        };
      } catch (e) {
...
async function lookup(code) {
  const { rows: [a] } = await query(
    `SELECT a.code, a.starts_at, a.ends_at, a.status, a.notes,
            s.name AS service_name, s.slug AS service_slug, s.price_from_cents, s.duration_min,
  ...
  return {
    ...
    service: { slug: a.service_slug, name: a.service_name, price_from_cents: a.price_from_cents, duration_min: a.duration_min },
